section .data
    ...
    shape_prompt db "Please select a shape (1-12):", 0xA
...
choice_done:
    ; Validate shape choice (1-12)
    cmp rax, 1
    jl show_error_and_restart  ; if choice < 1
    cmp rax, 12
    jg show_error_and_restart  ; if choice > 12
...
rectangle_outter:
    mov Hull    mov r14, r13            ; A copy of the width
...
draw_heart:
    ; Heart shape: two semicircles at top, downward triangle at bottom
    mov r12, 1                 ; Current row counter
    mov r15, [shape_size]      ; Total height of heart
    shr r15, 1                 ; Divide size by 2 for top semicircles
    inc r15                    ; Adjust for proper height

heart_top_outer:
    ; Print left semicircle
    mov r8, [shape_size]
    sub r8, r12                ; Leading spaces = size - current_row
    inc r8                     ; Extra space for symmetry

heart_top_spaces_left:
    cmp r8, 0
    je heart_top_stars_left
    mov rax, 1
    mov rdi, 1
    mov rsi, space
    mov rdx, space_len
    syscall
    dec r8
    jmp heart_top_spaces_left

heart_top_stars_left:
    ; Left semicircle: stars = 2 * current_row
    mov r13, r12
    add r13, r12               ; 2 * current_row

heart_top_stars_left_loop:
    mov rax, 1
    mov rdi, 1
    mov rsi, star
    mov rdx, star_len
    syscall
    dec r13
    jnz heart_top_stars_left_loop

    ; Middle space between semicircles
    mov rax, 1
    mov rdi, 1
    mov rsi, space
    mov rdx, space_len
    ...
    ; Right semicircle: same number of stars
    mov r13, r12
    add r13, r12               ; 2 * current_row

heart_top_stars_right:
    mov rax, 1
    mov rdi, 1
    mov rsi, star
    mov rdx, star_len
    ...
    inc r12
    cmp r12, r15               ; Compare with half size
    jle heart_top_outer

    ; Bottom triangle (inverted)
    mov r12, [shape_size]      ; Start from size
    mov r14, r15               ; Use half size as starting point

heart_bottom_outer:
    ; Leading spaces
    mov r8, [shape_size]
    sub r8, r14
    inc r8                     ; Adjust spacing

heart_bottom_spaces:
    cmp r8, 0
    je heart_bottom_stars
    mov rax, 1
    mov rdi, 1
    mov rsi, space
    mov rdx, space_len
...
heart_bottom_stars_loop:
    ...
    dec r14
    cmp r14, 0
    jg heart_bottom_outer

    jmp exit_program

draw_star:
    mov r12, 1                 ; Current row counter
    mov r15, [shape_size]      ; Total height
    shr r15, 1                 ; Divide by 2 for top half
    inc r15                    ; Adjust for proper height

star_top_outer:
    ; Leading spaces = (size - current_row)
    mov r8, [shape_size]
    sub r8, r12
...
star_top_stars_loop:
    ...
    ; Bottom part (inverted with middle line)
    mov r12, [shape_size]      ; Start from size
    mov r14, r15               ; Start from half size
